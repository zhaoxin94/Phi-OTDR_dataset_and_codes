...
def test(model, dataset, criterion):
    ...
    for (step, i) in enumerate(dataset):
        ...
        feature_list = torch.cat((feature_list, feature_label),
                                 dim=0)  # feature_list
...
def draw_result(C):
    ...
    sns.heatmap(df,
                ...
                annot_kws={'size': 10},
                xticklabels=[
                    'background', 'digging', 'knocking', 'watering', 'shaking',
                    'walking'
                ],
                yticklabels=[
                    'background', 'digging', 'knocking', 'watering', 'shaking',
    ...
    Acc = (C[0][0] + C[1][1] + C[2][2] + C[3][3] + C[4][4] +
           C[5][5]) / sum(C[0] + C[1] + C[2] + C[3] + C[4] + C[5])
    print('acc: %.4f' % Acc)
    NAR = (sum(C[0]) - C[0][0]) / sum(C[:, 1] + C[:, 2] + C[:, 3] + C[:, 4] +
                                      C[:, 5])
    print('NAR: %.4f' % NAR)
...
if __name__ == "__main__":
    ...
    '''model parameters'''
    rootpath = '/home/zhaoxin/data/das/opendas'
    parser.add_argument("--root",
                        type=str,
                        default=rootpath + '/train',
                        help="rootpath of traindata")
    parser.add_argument("--root2",
                        type=str,
                        default=rootpath + '/test',
                        help="rootpath of valdata")
    parser.add_argument("--txtpath",
                        type=str,
                        default=rootpath + '/train/label.txt',
                        help="path of train_list")
    parser.add_argument("--txtpath2",
                        type=str,
                        default=rootpath + '/test/label.txt',
